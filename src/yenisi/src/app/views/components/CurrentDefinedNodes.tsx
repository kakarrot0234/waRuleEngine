...
import { GridOptions, GridSizeChangedEvent, SelectionChangedEvent, GridReadyEvent, RowNode, ValueFormatterParams, } from 'ag-grid-community';
...
export function CurrentDefinedNodes (props: ICurrentDefinedNodesProps) {
  ...
  function getMasterGridOptionsCurrentMathNodes () {
    const gridOptions: GridOptions = {
      ...
      detailCellRendererParams: detailViewRender,
  ...
  function detailViewRender () {
    const detailGridProps = {
      detailGridOptions: {
        ...
        onGridReady: (gridReadyEventArgs: GridReadyEvent) => {
          gridReadyEventArgs.api.sizeColumnsToFit();
        },
        isRowMaster: (dataItem: IMathNode) => {
          return dataItem.OperandParameters != null && dataItem.OperandParameters.count > 0;
        },
      ...
      getDetailRowData: (detailRowDataEventArgs: {
                      node?: RowNode,
                      data?: any,
                      successCallback: (detailData: any[]) => void,
                  }) => {
        const parentNode = detailRowDataEventArgs.node!.data as IMathNode;

        if (parentNode.OperandParameters != null) {
          detailRowDataEventArgs.successCallback(parentNode.OperandParameters);
        }
      },
    };
    return detailGridProps;
  };
  ...
  return (
    <div className={`current-defined-nodes-root ${props.ClassName != null ? props.ClassName : ""}`}>
      <div className="math-nodes ag-theme-balham">
        <ZtDataGrid
          ref={ucGrdCurrentDefinedComplexNodes}
          dataSource={state.MathNodes != null ? state.MathNodes : []}
          ...
          detailCellRendererParams={state.MasterGridOptionsCurrentMathNodes!.detailCellRendererParams}
          ...
          onGridSizeChanged={state.MasterGridOptionsCurrentMathNodes!.onGridSizeChanged}
          getRowNodeId={state.MasterGridOptionsCurrentMathNodes!.getRowNodeId}></ZtDataGrid>
        <textarea
          style={{
            width: "100%",
          }}
