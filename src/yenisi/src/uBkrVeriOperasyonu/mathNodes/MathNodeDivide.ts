    ...
    return new Promise<any>(async (resolve, reject) => {
      try {
        ...
        if (this.OperandParameters[1].ResultData === 0) {
          throw new Error(`Result data of the Node that its id number is '${this.OperandParameters[1].FollowId}' is 0. The divider connot be 0.!`);
        }
        ...
        for (let index = 2; index < this.OperandParameters.length; index++) {
          ...
          if (nodeParameter.ResultData === 0) {
            throw new Error(`Result data of the Node that its id number is '${nodeParameter.FollowId}' is 0. The divider connot be 0.!`);
          }
