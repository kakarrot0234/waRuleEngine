...
import { Convert, } from "@zt/core-framework";
...
export abstract class MathNode implements IMathNode {
  ...
  constructor (props: IMathNodeCreatorProps) {
    ...
    this.ComplexMathExpression = props.ComplexMathExpression != null ? Convert.toString(props.ComplexMathExpression) : "";
  }

    public Guid: string | undefined;
    public FollowId: string | undefined;
    ...
    protected ValidateFindingResultIsSuccess (ruleNode: IMathNode) {
      if (ruleNode.ResultData == null) {
        throw new Error(`Finding Result data of the Node that its id number is '${ruleNode.FollowId}' was not success!`);
      }
    }
    protected ValidatePrecenceOfResultData (ruleNode: IMathNode) {
      if (ruleNode.ResultData == null) {
        throw new Error(`Result data of the Node that its id number is '${ruleNode.FollowId}' was not found!`);
      }
    }
    protected ValidateTypeOfResultDataIsNumber (ruleNode: IMathNode) {
      ...
      if (typeOfResultData !== "number") {
        throw new Error(`Type of Result Data of Node that its id number is '${ruleNode.FollowId}' must be of type '${typeOfResultData}'`);
      }
    }
    ...
    protected ValidateTypeOfResultDataIsAcceptable (listOfAcceptedTypes: string[], ruleNode: IMathNode) {
      ...
      if (!listOfAcceptedTypes.includes(typeOfResultData)) {
        ...
        throw new Error(`Type of Result Data of Node that its id number is '${ruleNode.FollowId}' must be one of type from: '${acceptedTypesStr}'`);
      }
    }
